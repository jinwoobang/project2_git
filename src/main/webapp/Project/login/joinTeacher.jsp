...
<script type="text/javascript">
	$(function(){
	   ...
	   $("#btn-join").click(function(){
	      ...
	      $("input[name='birth']").val($("select[name='birth1']").val()+'/'+$("select[name='birth2']").val()+'/'+$("select[name='birth3']").val());
	       //birth값+
	      ...
	      /*  if($("input[name='teachercode']").val()==''){
	    	  document.location.href="/web/eduSpring.do";
	      }else{
	    	  document.location.href="/web/eduSpring2.do";
	      } */ 
	     
	      
	      
	       /*  console.log($("input[name='name']").val()+" "+$("input[name='id']").val()+" "+$("input[name='pw']").val()+
	            " "+$("input[name='birth']").val()+" "+$("input[name='hp']").val()+" "+$("input[name='email']").val()+
	            " "+$("input[name='address']").val()+" "+$("select[name='pwq']").val()+" "+$("input[name='pwa']").val()); */  
	   });
	   ...
	   $("button[name='idcheck']").click(function(){
		...
		$("button[name='code']").click(function(){
			if($("input[name='teachercode']").val()!='TEACHERUU'){
				alert('강사코드를 다시 확인해 주세요.');
			}else{
				alert('확인되었습니다.');
			}
		});
...
<body cellpadding="0" cellspacing="0" marginleft="0" margintop="0" width="100%" height="100%" align="center">
   ...
   <div class="card align-middle" id="boxsize3">
      ...
      <div class="card-body">
      <form class="form-signin" method="POST" action="/web/eduSpring2.do" name="frm">
   ...
   <select name="email2" class="form-control" id="email"> 
      ...
      <input type="hidden" name="email" value="email">
        ...
        <h3><label id='joinlist'>강사코드</label></h3>
        <input type="text" id="sid" class="form-control" name="teachercode" placeholder="강사코드 ">
        <button id="btn-code" class="btn-code" type="button" name="code" >코드확인</button>
        <span id='notice'>*강사인 경우에만 입력하세요</span>
